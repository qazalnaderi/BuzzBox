...
class MediaService:
    ...
    async def create_media(self, file: UploadFile, email_address: str) -> MediaSchema:
        ...
        return MediaSchema(
            ...
            email_address=media.email_address,
            message="Media uploaded successfully",
        )

    async def __get_media_model(self, media_id: ObjectId, email_address: str) -> Tuple[
        ...
        if media.email_address != email_address:
            raise HTTPException(
    ...
    async def get_media(
            self, media_id: ObjectId, email_address: str
    ) -> tuple[MediaSchema, Callable[[], Generator[Any, Any, None]]]:
        media, file = await self.__get_media_model(media_id, email_address)
    ...
    async def get_media_data(self, media_id: str, email_address: str):
        _, file = await self.__get_media_model(ObjectId(media_id), email_address)
